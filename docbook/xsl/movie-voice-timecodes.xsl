...
<xsl:value-of select="@az:movieVoiceTimeCodeIn"/><xsl:text>&#09;</xsl:text><xsl:value-of select="@az:movieVoiceTimeCodeOut"/><xsl:text>&#09;</xsl:text><xsl:value-of select="normalize-space(.)"/><xsl:text>
</xsl:text>
</xsl:template>
